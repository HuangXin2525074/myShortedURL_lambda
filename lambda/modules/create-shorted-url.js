...
const businessLogic = async (params) => {
    ...
    try {
        ...
        if (!valid || !validateUrl(origUrl)) {
            return Responses._400({ status_code: 400,message: "error-request-data" });
        }
        ...
        const URL_Store = await Dynamo.put({shortUrl: shorted, count:count_number, origUrl: convert_url}, tableName).catch(err => {
